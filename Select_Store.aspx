<%@ Page Title="" Language="VB" MasterPageFile="~/Master.master" AutoEventWireup="false"
    CodeFile="Select_Store.aspx.vb" Inherits="Default2" %>
...
<asp:Image ID="Image2" runat="server" CssClass="VidePostLeft_Photo" ImageUrl='<%# Eval("photo","~\Content\Upload\images\imageOnPost\{0}") %>' Width="595" Height="420" />
</a>
</td>
<td>
...
<tr>
<td>
<div style="color:#fff;font:normal 18px titr;text-shadow:1px 1px 1px #000;text-align:center;padding:5px;border:4px solid #1a68ff;background-color:#368be1">
...
<a style="text-decoration:none;color:red" target="_blank" href='<%#  String.Format("../../store/{1}/{0}", Eval("title"), Eval("id")).ToString().Replace(" ","-")+".html" %>'>
مشاهده جزییات این بسته آموزشی </a>
</td>
</tr>
</table>
</div>
...
<asp:SqlDataSource ID="SDS_All" runat="server" ConnectionString="<%$ ConnectionStrings:GraphicMotionConnectionString %>" SelectCommand="SELECT * FROM [tbl_Learning_and_Films] WHERE ([Type] = @Type and flag=1 and category=1) ORDER BY [ID] DESC">
<SelectParameters>
